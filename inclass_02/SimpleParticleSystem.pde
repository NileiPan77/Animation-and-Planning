...
void update(float dt){
  ...
  if(shiftPressed){
    spherePos.add(obstacleVel.times(dt*2));
  }else{
  ...
  for (int i = 0; i <  numParticles; i++){
    ...
    if(life[i] >= 2){
      ...
      life[i] = life[numParticles-1];
      collide[i] = collide[numParticles-1];
      life[numParticles-1] = 0;
      numParticles--;
    }
    ...
    if (pos[i].distanceTo(spherePos) < (sphereRadius+r)){
      ...
      if(obstacleVel.length() >= 1){
        vel[i].add(obstacleVel.times(0.8));
      }
      collide[i] = true;
    }
  }
