...
float k_goal = 0.5;  //TODO: Tune this parameter to agent stop naturally on their goals
float k_avoid = 50;
float agentRad = 40;
float goalSpeed = 100;
...
void setup(){
  ...
  agentPos[3] = new Vec2(500,420);
  agentPos[4] = new Vec2(490,420);
  goalPos[0] = new Vec2(200,420);
  goalPos[1] = new Vec2(120,120);
  goalPos[2] = new Vec2(220,220);
  goalPos[3] = new Vec2(190,20);
  goalPos[4] = new Vec2(20,10);
...
float computeTTC(Vec2 pos1, Vec2 vel1, float radius1, Vec2 pos2, Vec2 vel2, float radius2){
  float ttc = rayCircleIntersectTime(pos2,radius2+radius1,pos1,vel1.minus(vel2));
...
Vec2 computeAgentForces(int id){
  ...
  Vec2 acc = goalVel.minus(agentVel[id]).times(k_goal);
  for(int i = 0; i < numAgents; i++){
    if(i != id){
        float ttc = computeTTC(agentPos[id],agentVel[id],agentRad,agentPos[i],agentVel[i],agentRad);
        if(ttc > 0){
            Vec2 idPos =  agentPos[id].plus(agentVel[id].times(ttc));
            Vec2 iPos = agentPos[i].plus(agentVel[i].times(ttc));
            Vec2 fDir = (idPos.minus(iPos)).normalized();
            acc.add(fDir.times(k_avoid * (1/ttc)));
        }
    }
  }
  return acc;
...
void draw(){
  ...
  for (int i = 0; i < numAgents; i++){
   ...
   //float ttc = computeTTC(agentPos[1],agentVel[1],agentRad,agentPos[0],agentVel[0],agentRad);
   //println("ttc between agent 0 and 1: ", ttc, " s."); 
   //float ttc2 = computeTTC(agentPos[1],agentVel[1],agentRad,agentPos[2],agentVel[2],agentRad);
   //println("ttc between agent 1 and 2: ", ttc2, " s."); 
